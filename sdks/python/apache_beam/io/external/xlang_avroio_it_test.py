...
class CrossLanguageAvroIOTest(unittest.TestCase):
  ...
  def test_xlang_avro_read(self):
      ...
      with TestPipeline() as p:
          assert_that(
              p | avroio.ReadFromAvro(path,use_fastavro=True),
              equal_to(RECORDS_DICT))

      ###option2
      with tempfile.NamedTemporaryFile() as dst:
          #path2 = dst.name
          path2 = "test_avro_sinkagfs"
          with TestPipeline() as p:
            # pylint: disable=expression-not-assigned
            p \
            | beam.Create(RECORDS_DICT) \
            | avroio.WriteToAvro(
                path2,
                SCHEMA,
                #codec='snappy',
      ...
      with TestPipeline() as p:
          ...
          assert_that(
              p
              | 'Read from Avro' >> ReadFromAvro(file_name, AvroWriteTestRow),
              equal_to(RECORDS)
          )

      ###need this for option 1 and option 2
      #with TestPipeline() as p:
      #  assert_that(
      #          p
      #         | 'Read from Avro' >> ReadFromAvro(path2, AvroWriteTestRow),
      #      equal_to(RECORDS)
      # )
